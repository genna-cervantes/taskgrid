...
const AIChat = ({ toggleAIChat }: { toggleAIChat: boolean }) => {
  ...
  const urlPattern =
    /^\/workspaces\/([A-Za-z0-9_-]+)\/projects\/([A-Za-z0-9-]+)$/;
  {
    const isInProjects = urlPattern.test(path);
        ...
        {toggleAIChat && !!projectId && (
          <ResizablePanel defaultSize={20} className="min-w-80">
            ...
            >
              ...
              >
                {messages.length <= 0 && <div className="flex flex-col h-full justify-center text-center px-4">
                    <h1 className="text-base font-bold text-fadedWhite">Meet Your AI Project Partner</h1>
                    <p className="text-xs mt-1 italic text-midWhite">Generate tasks from meeting notes, or simply ask What should I prioritize today?”</p>
                    </div>}
